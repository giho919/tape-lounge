...
const state={venue:'upbit',basis:'fx',only:false,andyOnly:false,query:'',sort:'volume',selected:null,tickers:{upbit:[],bithumb:[]},global:{},meta:[],fx:null,andy:null,errors:{},at:{},busy:false,timer:null,paint:null,controllers:new Set(),epoch:0,lastAttempt:0};
...
function render(){
 ...
 const filtered=rows.filter(r=>(!state.only||favorites.has(r.symbol))&&(!state.andyOnly||stages[r.symbol])&&(!state.query||(r.symbol+' '+name(r.symbol)).toLowerCase().includes(state.query))).sort((a,b)=>(b[state.sort]??-Infinity)-(a[state.sort]??-Infinity)||a.symbol.localeCompare(b.symbol));
