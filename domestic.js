...
const volume=x=>num(x)===null?'—':x>=1e12?fmt(x/1e12,2)+'조':x>=1e8?fmt(x/1e8,1)+'억':fmt(x/1e4)+'만';
...
const COLUMNS=[{label:'관심'},{key:'symbol',label:'종목',first:-1},{key:'price',label:'국내 현재가'},{key:'change',label:'전일 대비'},{key:'foreignKrw',label:'해외 환산가'},{key:'premium',label:'김프'},{key:'foreignChange',label:'해외 등락'},{key:'volume',label:'거래대금 · 24h'},{label:'비교 상태'}];
...
function render(){
 ...
 $('dm-head').innerHTML=COLUMNS.map(c=>{const t=esc(heading(c));if(!c.key)return `<th>${t}</th>`;const on=state.sort===c.key;return `<th${on?` aria-sort="${state.dir===1?'descending':'ascending'}"`:''}><button data-sort="${c.key}" title="${t} 기준 정렬">${t}<span aria-hidden="true">${on?(state.dir===1?'▼':'▲'):'⇅'}</span></button></th>`;}).join('');
 ...
 $('dm-table').innerHTML=filtered.length?filtered.map(r=>`<tr><td><button data-star="${r.symbol}" aria-label="${r.symbol} 관심 코인" aria-pressed="${favorites.has(r.symbol)}">${favorites.has(r.symbol)?'★':'☆'}</button></td><td>${rowButton(r)}<small>${esc(name(r.symbol))}</small>${stages[r.symbol]?`<span class="dm-badge">Andy · ${esc(stages[r.symbol])}</span>`:''}</td><td>${krw(r.price)}${r.fromHigh===null?'':`<small>고가 대비 ${pct(r.fromHigh)}</small>`}</td><td class="${color(r.change)}">${pct(r.change)}</td><td>${krw(r.foreignKrw)}${num(r.foreign)===null?'':`<small>${fmt(r.foreign,6)} USDT</small>`}</td><td class="${color(r.premium)}">${pct(r.premium)}${r.gap===null?'':`<small>${signedKrw(r.gap)}</small>`}</td><td class="${color(r.foreignChange)}">${pct(r.foreignChange)}</td><td>${volume(r.volume)}원${num(r.foreignVolume)===null?'':`<small>해외 ${usdt(r.foreignVolume)} USDT</small>`}</td><td class="${r.reason?'dm-warn':'dm-muted'}">${esc(r.reason||'비교 가능')}<small>국내 체결 ${r.at?time(r.at):'—'}</small></td></tr>`).join(''):`<tr><td colspan="9">${state.only?'관심 코인의 별표를 눌러 나만의 목록을 만들어 보세요.':'검색 결과가 없거나 시세를 조회하지 못했습니다.'}</td></tr>`;
