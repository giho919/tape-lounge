...
function rowsFor(tickers,global,fx,basis,now=Date.now()){
 ...
 return tickers.filter(t=>t.market?.startsWith('KRW-')&&safeSymbol(t.market.slice(4))).map(t=>{
  ...
  return {symbol,price,volume:num(t.acc_trade_price_24h),change:num(t.signed_change_rate)===null?null:num(t.signed_change_rate)*100,at,current,
   high,low:num(t.low_price),fromHigh:positive(high)&&positive(price)?(price/high-1)*100:null,
   foreign,foreignKrw:converted,foreignChange:positive(open)&&positive(foreign)?(foreign/open-1)*100:null,foreignVolume:remote?num(g.quoteVolume):null,
...
const volume=x=>num(x)===null?'—':(x>=1e12?fmt(x/1e12,2)+'조':x>=1e8?fmt(x/1e8,1)+'억':fmt(x/1e4)+'만')+'원';
...
<div class="dm-tools"><label>국내 거래소<select id="dm-venue"><option value="upbit">업비트</option><option value="bithumb">빗썸</option></select></label><label>가격차 환산 기준<select id="dm-basis"><option value="fx">원달러 일일 기준 · 근사 김프</option><option value="usdt">선택 거래소 USDT 가격</option></select></label><label>종목 찾기<input id="dm-search" type="search" placeholder="BTC, 비트코인…" maxlength="40"></label><button id="dm-favorites" aria-pressed="false">☆ 관심 코인</button></div>
<div class="dm-andybar"><button id="dm-andy-only" aria-pressed="false">Andy 후보만</button><span id="dm-andy-at">Andy 장부 확인 중…</span><a href="reports/andy_scan.html#andy-retest" target="_blank" rel="noopener">장부 ↗</a></div>
<div id="dm-detail" hidden class="dm-detail"></div>
<p id="dm-count"></p><div class="dm-scroll" tabindex="0" role="region" aria-label="국내 시세 표, 가로 스크롤 가능"><table><thead><tr id="dm-head"></tr></thead><tbody id="dm-table"></tbody></table></div>
<details><summary>계산 기준과 데이터 주의사항</summary><p>표 읽는 법: 해외 환산가는 Binance 현물 USDT 가격에 선택한 환산 기준을 곱한 원화 값이고, 김프 칸 아래 작은 숫자는 국내 가격에서 그 환산가를 뺀 금액입니다. 고가 대비는 24시간 고가 대비 현재가의 위치, 거래대금 칸의 작은 숫자는 해외 24시간 거래대금(USDT)입니다. 머리글을 누르면 정렬 기준과 오름·내림 방향이 바뀝니다.</p><p>원달러 기준: (국내 원화 가격 ÷ (Binance 현물 USDT 가격 × 일일 USD/KRW 기준환율) − 1) × 100. 1 USDT ≈ 1 USD를 가정한 근사치로, 테더의 달러 가격차가 포함될 수 있습니다. 기준환율은 실시간 환율이 아니며 주말·휴일에는 마지막 고시일을 사용합니다.</p><p>USDT 기준: (국내 원화 가격 ÷ (Binance 현물 USDT 가격 × 선택 거래소 USDT/KRW 가격) − 1) × 100. 원달러 기준 김프와 다른 값입니다. 양쪽 가격은 같은 순간의 호가가 아닌 최근 체결가입니다.</p><p>시세는 WebSocket으로 수신해 화면에 1초 단위로 묶어 반영합니다. 연결이 끊기면 30초 주기 조회로 전환하며, 국내장을 떠나면 연결을 닫습니다. 최초 연결이나 재연결은 요청 제한 때문에 수 초 이상 걸릴 수 있습니다. 마지막 국내 체결·해외 시세가 2분을 넘거나 기준환율이 7일을 넘으면 가격차 계산에서 제외합니다. 이전 가격은 지연 표시로 남을 수 있습니다.</p><p>동일 티커 기준 비교이며 토큰·네트워크의 동일성과 입출금 가능 여부를 보증하지 않습니다. 입출금 상태는 미확인입니다. 수수료·슬리피지·송금 지연을 반영한 차익거래 수익이 아니며, 큰 가격차만 보고 거래하지 마세요. ±50% 초과 값은 재확인 대상으로 계산값을 숨깁니다.</p><p>출처: Upbit·Bithumb 공개 시세, Binance 현물, Frankfurter 일일 환율, Tape Lounge Andy 리포트. 관심 코인은 이 브라우저에만 저장됩니다.</p></details>`;
...
const COLUMNS=[{label:'관심'},{key:'symbol',label:'종목',first:-1},{key:'premium',label:'김프',group:1},{key:'price',label:'국내 현재가',group:1},{key:'change',label:'전일 대비'},{key:'foreignKrw',label:'해외 환산가',group:1},{key:'foreignChange',label:'해외 등락'},{key:'volume',label:'거래대금 · 24h',group:1},{label:'비교 상태'}];
function heading(c){return c.key==='premium'?(state.basis==='fx'?'근사 김프':'USDT 기준 가격차'):c.label;}
...
function render(){
 ...
 $('dm-status').textContent=`국내 ${live[state.venue]?'실시간 수신':'연결 대기·30초 조회'} · 해외 ${live.binance?'실시간 수신':'연결 대기·30초 조회'} · 화면 1초 반영`+(state.at[state.venue]?` · ${time(state.at[state.venue])} KST`:'')+(failures.length?' · 일부 조회 실패: '+failures.map(k=>({upbit:'업비트',bithumb:'빗썸',binance:'Binance',fx:'환율'}[k])).join(', '):'');
 $('dm-head').innerHTML=COLUMNS.map(c=>{const t=esc(heading(c));const g=c.group?' class="dm-g"':'';if(!c.key)return `<th${g}>${t}</th>`;const on=state.sort===c.key;return `<th${g}${on?` aria-sort="${state.dir===1?'descending':'ascending'}"`:''}><button data-sort="${c.key}" title="${t} 기준 정렬">${t}<span aria-hidden="true">${on?(state.dir===1?'▼':'▲'):'⇅'}</span></button></th>`;}).join('');
 ...
 $('dm-count').textContent=`${state.venue==='upbit'?'업비트':'빗썸'} 원화마켓 · ${filtered.length}종 · 표 머리글을 누르면 정렬 기준과 방향을 바꿀 수 있고, 종목을 누르면 연결 정보를 볼 수 있습니다.${storageWarning?' 브라우저 저장이 차단되어 관심 목록은 이번 방문 동안만 유지됩니다.':''}`;
 $('dm-table').innerHTML=filtered.length?filtered.map(r=>`<tr><td><button data-star="${r.symbol}" aria-label="${r.symbol} 관심 코인" aria-pressed="${favorites.has(r.symbol)}">${favorites.has(r.symbol)?'★':'☆'}</button></td><td>${rowButton(r)}<small>${esc(name(r.symbol))}${stages[r.symbol]?` <span class="dm-badge">Andy · ${esc(stages[r.symbol])}</span>`:''}</small></td><td class="dm-g dm-kimp ${color(r.premium)}"><b>${pct(r.premium)}</b>${r.gap===null?'':`<small>${signedKrw(r.gap)}</small>`}</td><td class="dm-g dm-num">${krw(r.price)}${r.fromHigh===null?'':`<small>고가 대비 ${pct(r.fromHigh)}</small>`}</td><td class="${color(r.change)}">${pct(r.change)}</td><td class="dm-g">${krw(r.foreignKrw)}${num(r.foreign)===null?'':`<small>${fmt(r.foreign,6)} USDT</small>`}</td><td class="${color(r.foreignChange)}">${pct(r.foreignChange)}</td><td class="dm-g">${volume(r.volume)}${num(r.foreignVolume)===null?'':`<small>해외 ${usdt(r.foreignVolume)} USDT</small>`}</td><td class="dm-state ${r.reason?'dm-warn':'dm-muted'}">${r.reason?`<span class="dm-chip">${esc(r.reason)}</span>`:'비교 가능'}<small>체결 ${r.at?time(r.at):'—'}</small></td></tr>`).join(''):`<tr><td colspan="9">${state.only?'관심 코인의 별표를 눌러 나만의 목록을 만들어 보세요.':'검색 결과가 없거나 시세를 조회하지 못했습니다.'}</td></tr>`;
