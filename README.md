...
- **Leverage · 레버리지** — isolated-margin positions at 1–10×; the panel shows entry price, liquidation price, margin, and unrealized PnL with ROE, exchange style. A candle whose wick touches the liquidation price wipes the position's margin, and a wiped account settles the run immediately / 1~10배 격리 마진 포지션. 거래소처럼 진입가·청산가·증거금·미실현 손익(ROE)을 함께 표시하며, 캔들 고저가가 청산가에 닿으면 증거금이 전액 소멸하고 밑천이 다 녹으면 즉시 정산
